...
namespace Authing.CSharp.SDK.Services
{
    public partial class AuthenticationClient
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(m_AppHost))
            {
        ...
        private string BuildOauthAuthorizeUrl(OauthOption option)
        {
            ...
            return $"{m_AppHost}/oauth/auth{param}";
        }
        ...
        private string BuildCasAuthorizeUrl(CasOption option)
        {
            return option.Service is null
                ? $"{options.AppHost}/cas-idp/{options.AppId}/login"
                : $"{options.AppHost}/cas-idp/{options.AppId}/login?service={option.Service}";
        }

        private string BuildSamlAuthorizeUrl()
        {
            return $"{options.AppHost}/api/v2/saml-idp/{options.AppId}";
        }
        ...
        public async Task<ValidateTokenResponse> ValidateToken(ValidateTokenParams param)
        {
            ...
            var result = await GetWithHostAsync(m_AppHost, url).ConfigureAwait(false);
        ...
        private string BuildEasyLogoutUrl(LogoutParams options)
        {
            return string.IsNullOrWhiteSpace(options.RedirectUri)
                ? $"{m_AppHost}/login/profile/logout"
                : $"{m_AppHost}/login/profile/logout?redirect_uri={options.RedirectUri}";
        }
        ...
        private string BuildOidcLogoutUrl(LogoutParams options)
        {
            ...
            return string.IsNullOrWhiteSpace(options.RedirectUri)
                ? $"{m_AppHost}/oidc/session/end"
                : $"{m_AppHost}/oidc/session/end?id_token_hint={options.IdToken}&post_logout_redirect_uri={options.RedirectUri}";
        }
        ...
        private async Task<string> RevokeTokenWithClientSecretBasic(string url, string token)
        {
            ...
            var result = await GetWithHostAsync(m_AppHost,
