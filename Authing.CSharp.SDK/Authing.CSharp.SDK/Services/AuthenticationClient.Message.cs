...
namespace Authing.CSharp.SDK.Services
{
    public partial class AuthenticationClient
    {
        ...
        {
            string json = await PostAsync("/api/v3/send-sms", m_JsonService.SerializeObjectCamelCase(param)).ConfigureAwait(false);
        ...
        {
            string json = await PostAsync("/api/v3/send-email", m_JsonService.SerializeObjectCamelCase(param)).ConfigureAwait(false);
            CommonResponseDto res = jsonService.DeserializeObject<CommonResponseDto>(json);
            return res;
        }
    }
